...
VideoWindow::VideoWindow(int argc, char* argv[], QWidget *parent) : QWidget(parent) {
    ...
    statusLabel->setStyleSheet("background-color: #222; color: #aaa; padding: 5px; font-style: italic;");
    ...
    hwAccelBox->setStyleSheet("color: white; background-color: #111; padding: 4px;");
    layout->addWidget(hwAccelBox);
    connect(hwAccelBox, &QCheckBox::toggled, [this](bool checked) {
        player.use_hw_accel = checked ? 1 : 0;
        if (player.uri) load_new_video(QString(player.uri));
    });
    ...
    if (argc > 1) {
        QTimer::singleShot(250, [this, argv]() {
            load_new_video(QString(argv[1]));
        });
    }
}
...
void VideoWindow::mainFileMenu(QMenuBar*menuBar) {
    ...
    connect(openAction, &QAction::triggered, this, &VideoWindow::open_file_dialog);
...
void VideoWindow::helpMenu(QMenuBar*menuBar) {
    ...
    connect(aboutAction, &QAction::triggered, [this]() {
        ...
        QMessageBox::about(this, tr("About Mini Timeline View"),
...
void VideoWindow::playbackMenu(QMenuBar *menuBar) {
    ...
    connect(volUpAction, &QAction::triggered, [this]() {
    ...
    connect(volDownAction, &QAction::triggered, [this]() {
        change_volume(&player, -0.10);
    });
...
void VideoWindow::load_new_video(const QString &fileName) {
    ...
    statusLabel->setStyleSheet("background-color: #333; color: #FFFFFF; padding: 5px;");
...
void VideoWindow::probeAudio(AVFormatContext *streamCtx) {
        ...
        for (unsigned int i = 0; i < streamCtx->nb_streams; i++) {
            ...
            AVDictionaryEntry *lang  = av_dict_get(streamCtx->streams[i]->metadata,
                                                   "language", NULL, 0);
            AVDictionaryEntry *title = av_dict_get(streamCtx->streams[i]->metadata,
                                                   "title", NULL, 0);
...
void VideoWindow::open_file_dialog() {
    QStringList video_extensions = {
        "mp4", "mkv", "mov", "avi", "webm", "flv", "vob", "ogv", "ogg",
        "wmv", "mpg", "mpeg", "m4v", "3gp", "3g2", "mxf", "f4v"
    };
    ...
    QString fileName = QFileDialog::getOpenFileName(this, tr("Open Video"), "", filter);
